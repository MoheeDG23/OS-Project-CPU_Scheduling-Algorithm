...
void fcfs(vector<int> &arrival_time, vector<int> &burst_time,vector<int> &completion_time)
{
	...
	for(int i=1;i<num_proc;i++)
    {
		if(completion_time[i-1]>arrival_time[i])
			...
			completion_time[i]=(arrival_time[i]-completion_time[i-1])+burst_time[i]+completion_time[i-1];
	}
}
...
int main()
{
    ...
    fcfs(arrival_time, burst_time, completion_time);
