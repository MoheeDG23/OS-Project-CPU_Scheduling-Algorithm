...
execution_time = period = remain_time = deadline = remain_deadline = burst_time = wait_time = completion_time = arrival_time = [0  for i in range (MAX_PROCESS)]
...
def round_robin(time):
	...
	i=0, current_time = 0, temp_process=0, flag = 0, count = 0, total_wait = 0, total_end = 0
	process_list = [0 for i in range(time)], process_cycle = [-1 for i in range(time)]
    avg_wait=float(0), avg_end=float(0)
	...
	while(temp_process):
  	...
  	proc = 0, update_process_cycle=[0 for i in range(time)]
